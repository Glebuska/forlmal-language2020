...
def test_language2():
    script2 = "select edges from query * term 'db_1'"
...
def test_language5():
    script5 = "select edges from query * | nonterm 'str1' term 'db'"
...
def test_pattern_name_term():
    script_true = "nameofpattern 'pattern1' term 'a1'"
    script_false = "nameofpattern 'pattern1' 'a1'"
    assert query_script(script_true, path)
    assert not query_script(script_false, path)


def test_vertices_nums_and_none():
    script_true = "select edges from start 1 2 3 final none for 'graph1'"
    script_false = "select edges from start someBad. final none for 'graph1'"
    assert query_script(script_true, path)
    assert not query_script(script_false, path)


def test_vertices_range():
    script_true = "select edges from start range from 1 to 9 final none for 'graph1'"
    script_false = "select edges from range from 2 to 'BAD' none for 'graph1'"
    assert query_script(script_true, path)
    assert not query_script(script_false, path)


def test_vertices_cond_eq():
    script_true = "select filter 'g1' 'g2' 'g3' isEquel 'str4' 'str5' edges from 'graph'"
    assert query_script(script_true, path)


def test_vertices_cond():
    script_true = "select filter 'g1' 'g2' 'g3' ((isStart 'str4' or isStart 'str4') and isFinal 'str5') edges from " \
                  "'graph' "
    assert query_script(script_true, path)
